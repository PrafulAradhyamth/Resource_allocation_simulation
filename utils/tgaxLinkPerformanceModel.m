...
%   mcs = 3;
%   coding = 'BCC';
%   per = estimateLinkPerformance(abstraction,sinrVal,dataLen,'HE_SU',mcs,coding);
%
...
methods
    ...
    function [per,snreff] = estimateLinkPerformance(obj,sinr,varargin)
        ...
        %
        % [PER,SNREFF] = estimateLinkPerformance(OBJ,SINR,CFGSU) returns
        % the estimated packet error rate given the single-user format
    ...
    function [snreff,scrbir,avrbir] = effectiveSINR(obj,sinr,format,mcs,varargin)
        ...
        if ~coder.target('MATLAB')
            rbirIdx = find(modscheme==obj.rbir.tableModScheme);
            rbirIdx = rbirIdx(1);
            % Workaround for obj.rbir.rbirTable being heterogeneous cell
            % array and therefore cannot index into it with a non-constant
            % at runtime
            for i = 1:numel(obj.rbir.rbirTable)
                rbirSNR = obj.rbir.rbirTable{i}(:,1); % SNR first column
                rbirVal = obj.rbir.rbirTable{i}(:,2); % RBIT second column
                if i==rbirIdx
                    break;
                end
            end
        else
            rbirIdx = modscheme==obj.rbir.tableModScheme;
            rbirSNR = obj.rbir.rbirTable{rbirIdx}(:,1); % SNR first column
            rbirVal = obj.rbir.rbirTable{rbirIdx}(:,2); % RBIT second column
        end

        % Tuning
        sinr = sinr/beta;

        % Limit to range of LUT
        sinr(sinr<obj.rbir.minSNR(rbirIdx)) = obj.rbir.minSNR(rbirIdx);
        sinr(sinr>obj.rbir.maxSNR(rbirIdx)) = obj.rbir.maxSNR(rbirIdx);
        ...
        snreff = alpha*interp1(rbirVal,rbirSNR,avrbir);
    end
...
function [modscheme,rate] = mcs2rate(format,mcs)
    ...
    switch format
        ...
        case {'VHT','HE_SU','HE_EXT_SU','HE_MU','HE_TB'}
