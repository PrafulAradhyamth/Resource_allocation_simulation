function [totalTxTime, latencyInformation] = assignmentAlgorithmLatencyMultiTXOP(config, stationInformation, sortFlag)
% Assignment algorithm (latency; multi TXOP) : Modified version of the
% assignment algorithm used for latency analysis in Multi TXOP simulations.
...
if sortFlag == "FS"
    % Sort stationInformation table by MaximumLatency and FrameSize columns
    stationInformation = sortrows(stationInformation, ["MaximumLatency", "FrameSize"]);
...
while numRemainingSTA > 0
    ...
    while 1
        ...
        % Compute minimal PPDU transmission time over different RU
        % configurations
        [solution, objectiveValue, ruPartition, userRUMCS, userRUSNR, allocationIndex] = PPDUTxTimeAssignmentAlgorithm(numSTAToSchedule, stationInformationTemp, SINRs, mcsConfig.table_mcs, mcsConfig.target_PDR, userConfig);
        
        % Check if maximum latency constraints would be satisfied for each
        % station (Here the maximum latency constraints are relaxed;
        % multiplied by the generosity factor)
        feasibleFlag = 1;
        for idx = 1 : numSTAToSchedule
            if stationInformationTemp.TimeSinceGeneration(idx) + constantsConfig.triggerFrame + constantsConfig.SIFS + objectiveValue > generosityFactor * stationInformationTemp.MaximumLatency(idx)
                feasibleFlag = 0;
            end
        end
    ...
    if feasibleFlag
        ...
        [stationSchedule, MCSSchedule, ~] = assignmentMtxToSTASchedule(solution.x, userRUMCS, userRUSNR, stationInformationTemp);
